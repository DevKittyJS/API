...
function loadIcons(){const styleSheetId="DevKittyIcons";const iconsBaseUrl="https://raw.githubusercontent.com/DevKittyJS/API/main/icons";const version=Date.now();let styleSheet=document.getElementById(styleSheetId);if(!styleSheet){styleSheet=document.createElement("style");styleSheet.id=styleSheetId;document.head.appendChild(styleSheet)}
styleSheet.textContent="";const addedIcons=new Set();const baseStyles=`
    .dk_ico {
  ...
  `;styleSheet.appendChild(document.createTextNode(baseStyles));function supportsMask(){return(CSS.supports("mask-image","url('dummy.svg')")||CSS.supports("-webkit-mask-image","url('dummy.svg')"))}
if(!supportsMask()){console.error("[DevKitty] Masking not supported. Removing icons.");document.querySelectorAll(".dk_ico").forEach((el)=>el.remove());return}
const icons=document.querySelectorAll(".dk_ico");icons.forEach((el)=>{const iconClass=Array.from(el.classList).find((c)=>c.startsWith("i-"));if(!iconClass)return;const iconName=iconClass.slice(2);const folderClass=Array.from(el.classList).find((c)=>c.startsWith("dk-")&&c!=="dk_ico");if(!folderClass){console.warn(`[DevKitty] Icon "${iconName}" is missing a folder class.`);return}
const folderName=folderClass.slice(3);const cacheKey=`${folderName}/${iconName}`;if(addedIcons.has(cacheKey))return;addedIcons.add(cacheKey);const url=`${iconsBaseUrl}/${folderName}/${iconName}.svg?v=${version}`;const iconRule=`
      .dk_ico.${folderClass}.i-${iconName} {
        -webkit-mask-image: url('${url}');
                mask-image: url('${url}');
      }
    `;styleSheet.appendChild(document.createTextNode(iconRule));console.log(`[DevKitty] Icon loaded: ${url}`)})}
document.addEventListener("DOMContentLoaded",()=>{loadScreens();loadIcons()})